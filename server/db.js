...
// module.exports = { connectToDatabase, addUser };
...
// // Create MySQL connection pool to docker-file upload
// const pool = mysql.createPool({
//   connectionLimit: 10,
//   host: "db",
//   user: "root",
...
function returnBook(bookId, userId, callback) {
  ...
  const checkQuery = "SELECT * FROM issue_books WHERE bookId = ? AND userId = ?";
  pool.query(checkQuery, [bookId, userId], (err, results) => {
    ...
    const deleteQuery = "DELETE FROM issue_books WHERE bookId = ? AND userId = ?";
...
module.exports = { pool,connectToDatabase, createBooksTable, createIssueBooksTable, createUsersTable, addUser, addBook, issueBook, returnBook };
